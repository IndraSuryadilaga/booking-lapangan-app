        ...
        <div class="max-w-4xl space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-neutral-200 pb-5">
                <div>
                    ...
                    <p class="mt-2 text-sm text-neutral-500 font-normal">Perbarui informasi lapangan, jam operasional harian, galeri foto, dan tarif per slot.</p>
                </div>
                <div class="shrink-0">
            ...
            <div class="bg-white border border-neutral-200/60 rounded-2xl shadow-sm p-6 space-y-6">
                <h2 class="text-base font-bold text-neutral-800">Galeri Foto Lapangan</h2>
                            ...
                            <div class="w-full flex flex-col gap-2">
                                ...
                                @else
                                    <form action="{{ route('admin.fields.images.primary', [$field->id, $image->id]) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" class="w-full bg-primary-100/50 hover:bg-primary-100 text-primary-600 text-xs font-bold py-1.5 rounded-lg transition-colors border border-primary-200/20">
                                            Jadikan Utama
                                        </button>
                                    </form>
                                @endif
                                ...
                                <form action="{{ route('admin.fields.images.destroy', $image->id) }}" method="POST" onsubmit="return confirm('Yakin mau hapus foto ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-full bg-danger-100/50 hover:bg-danger-100 text-danger-700 text-xs font-bold py-1.5 rounded-lg transition-colors border border-danger-200/20">
                                        Hapus
                                    </button>
                                </form>
                            </div>
                ...
                <!-- Upload New Photos -->
                <form action="{{ route('admin.fields.images.store', $field->id) }}" method="POST" enctype="multipart/form-data" class="pt-4 border-t border-neutral-100 flex flex-col sm:flex-row items-end gap-4">
                    @csrf
                    <div class="flex-1 w-full">
                        <label class="block text-sm font-bold text-neutral-700 mb-2">Tambah Foto Baru</label>
                    ...
                    <div class="border-t border-neutral-100 pt-6">
                        ...
                        <div class="space-y-4">
                            @foreach($days as $index => $day)
                                ...
                                @endphp
                                <div x-data="{ isOpen: {{ $isOpen }} }" class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 border border-neutral-200/50 rounded-2xl bg-neutral-50/30">
                                    ...
                                    <div class="flex items-center gap-4">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <input type="time" name="operating_hours[{{ $index }}][open_time]"
                                                   value="{{ $data ? \Carbon\Carbon::parse($data->open_time)->format('H:i') : '08:00' }}" :disabled="!isOpen" class="border border-neutral-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:bg-neutral-100 disabled:text-neutral-400 shadow-sm font-semibold">
                                        </div>
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            <input type="time" name="operating_hours[{{ $index }}][close_time]"
                                                   value="{{ $data ? \Carbon\Carbon::parse($data->close_time)->format('H:i') : '22:00' }}" :disabled="!isOpen" class="border border-neutral-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:bg-neutral-100 disabled:text-neutral-400 shadow-sm font-semibold">
                                        </div>
                                    </div>
                                </div>
                            @endforeach
